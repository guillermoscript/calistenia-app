...
Status: DONE (v1, 2026-06) and then **upgraded to mcp-use v2 (2026-08, #442)** — see the addendum right below. Everything after the addendum describes the original SDK+Express → mcp-use **v1** migration and is kept as history; several v1 APIs it names (`mcp-use/server`, `useWidget`, `McpUseProvider`, `--widgets-dir`, `widget: {}` on tools) no longer exist.

## Addendum — mcp-use v1.34.6 → v2.2.3 (2026-08-15, issue #442)

What changed in this repo, so nobody re-derives it from the framework's migration guides (which over- and under-state the work — see the issue for the audit):

- **Imports.** `mcp-use/server` is gone: server APIs come from `mcp-use`, `oauthCustomProvider` from `mcp-use/oauth`, React hooks from `mcp-use/react`. Every `registerXTools(server, …)` takes `AppServer = MCPServer<BridgeUser>` (exported from `src/mcpuse/auth-bridge.ts`) because v2's `MCPServer` is generic on the OAuth user type.
- **Entry / lifecycle.** `src/server.ts` only registers and `export default server`. `mcp-use dev|build|start` own `listen()`; `src/bootstrap.ts` (side-effect import) owns the reminder scheduler + SIGINT/SIGTERM and skips itself under `mcp-use build|typecheck` (the CLI evaluates the entry to collect tool refs). `src/standalone.ts` = `npm run dev:simple` (plain `tsx`, no Views).
- **Auth bridge rewritten** on the v2 `oauthCustomProvider` shape: `createTokenVerifier(resource)` wraps the same dual-auth (our OAuth store, then PocketBase JWT) into an SDK `OAuthTokenVerifier` (must return `expiresAt` **and** `resource`, or the bearer gate answers 401 "did not return a validated protected resource"); `oauthMetadata` advertises our own `/authorize`, `/token`, `/register`, `/revoke`; `mapAuthInfo` rebuilds `ctx.auth.user` (`BridgeUser`) from `AuthInfo.extra`. MCP middleware (`server.use("mcp:*")`) sees the raw `AuthInfo`, not the mapped user. Own Hono OAuth/API routes untouched; the framework additionally serves `/.well-known/oauth-protected-resource/mcp`.
- **Config.** `baseUrl` is gone (public origin = `MCP_URL`, set from `SERVER_URL` in `server.ts`); `cors` fields are `methods`/`allowedHeaders`.
- **Widgets → Views.** `resources/<name>.tsx` → `views/<name>/view.tsx`, `resources/lib/` → `views/lib/` (no `view.tsx` inside, so the CLI ignores it). Per View: `useWidget()` → `useToolContext()` with explicit `pending`/`error`/`ready` branches (`WidgetLoading` / `WidgetError` in `views/lib/ui.tsx`), no `McpUseProvider` wrapper, `useWidgetTheme` → `useViewTheme`, `sendFollowUpMessage(x)` → `useSendFollowUp()({ prompt: x })`, `useCallTool` handle is `{ callTool, data, error, isPending }` and `callTool()` rejects on tool error. Fonts resolve via `getPublicBaseUrl()`.
- **View-bound tools** declare `view: { name }` **and** `outputSchema` — v2 throws at registration without it — using the zod schema that used to live in the widget (`src/views/<name>.schema.ts`, single source of truth for both sides). They return raw envelopes via `viewResult(props, text)` (`src/utils.ts`) or `errorResult()`: with an `outputSchema`, `widget()`/`error()` no longer type-check and content-only results are rejected at runtime. `invoking`/`invoked` labels are dropped (no slot in v2's `ToolViewConfig`). Non-view tools still use `schema:`/`widget()`/`object()`/`text()` unchanged.
- **Build / deploy.** `mcp-use build` → `.mcp-use/build/{index.js,manifest.json,views/}` (deps stay external → runtime still `npm ci --omit=dev`); `mcp-use start` serves it and reads `PORT`/`HOST` from env (defaults 3000 / 127.0.0.1 → the Dockerfile sets `PORT=3001 HOST=0.0.0.0`). `mcp-env.d.ts` and `.mcp-use/` are generated and git-ignored. Node ≥ 22.22.2; `react`/`react-dom` 19 are explicit devDeps.
- **Typing of Views.** `tsconfig.views.json` (part of `npm run typecheck`) type-checks `views/**` against the schema modules; run `npm run build` once first so `mcp-env.d.ts` exists. `server.ts` exports the two tool refs Views call through `useCallTool` (`logFullWorkout`, `setCurrentProgram`), which is what gives those calls real input types — and what exposed that the session-tracker "Registrar sesión" button had been sending `{ workout_key, notes }` to a tool whose schema requires `phase`/`day`/`exercises` (fixed on the way). `useToolContext()` still casts `toolOutput as Props` because the view-bound tools' refs are not exported (they are registered inside `registerXTools()`), which is fine: the schema module is shared by both sides.
- **Not done (follow-ups).** `schema:` → `inputSchema:` on the 67 non-view tools; exporting the 19 view tool refs so `useToolContext<"name">()` infers `toolOutput` without the cast.
