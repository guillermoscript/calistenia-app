...
export default function AchievementsGrid() {
  ...
  const accent = "#6366f1";
  const dimBg = dark ? "#1e1e1e" : "#f0f0f0";

  if (isPending) {
    return (
      <McpUseProvider autoSize>
        <div style={{ padding: 16, color: dark ? "#e0e0e0" : "#333" }}>Cargando logros…</div>
      </McpUseProvider>
    );
  }
    ...
    <McpUseProvider autoSize>
      <div style={{ padding: 16, backgroundColor: bg, color: textColor, fontFamily: "system-ui, sans-serif", maxWidth: 480 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          ...
          {level !== undefined && (
            <div style={{ backgroundColor: card, borderRadius: 10, padding: "8px 12px", border: `1px solid ${border}`, textAlign: "center" }}>
              <div style={{ fontSize: 11, color: sub, marginBottom: 2 }}>Nivel</div>
              <div style={{ fontWeight: 800, fontSize: 18, color: accent }}>{level}</div>
              {xp !== undefined && (
                <div style={{ fontSize: 10, color: sub }}>{xp.toLocaleString()} XP</div>
              )}
            </div>
          )}
        </div>
        ...
        {level_progress_pct !== undefined && xp_to_next_level !== undefined && (
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ height: 6, borderRadius: 4, backgroundColor: dark ? "#333" : "#e0e0e0", overflow: "hidden" }}>
              <div
                style={{
                  height: "100%",
            ...
            <div key={cat} style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 8 }}>
                {achs.map((a) => {
                  ...
                  const badgeBg = isLocked ? dimBg : card;
                  const iconSize = 44;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 10,
                        border: `1px solid ${a.unlocked ? tierColor + "55" : border}`,
                        padding: "10px 8px",
                        display: "flex",
                    ...
                    >
                      ...
                      <div style={{ position: "relative", width: iconSize, height: iconSize, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        {a.unlocked && (
                          <span style={{
                            ...
                            fontSize: 13, backgroundColor: bg, borderRadius: "50%", lineHeight: 1,
                          }}>✅</span>
                        )}
                      </div>
                      ...
                      <div style={{
                        ...
                        fontSize: 11,
                        textAlign: "center",
                        color: a.unlocked ? textColor : sub,
                        lineHeight: 1.3,
                      }}>
                        {a.name}
                      </div>
                      ...
                      {isLocked && a.progress_pct > 0 && (
                        <div style={{ width: "100%", height: 4, borderRadius: 3, backgroundColor: dark ? "#333" : "#ddd", overflow: "hidden" }}>
                          <div style={{
                            height: "100%",
                      ...
                      {/* Progress pct label */}
                      {isLocked && (
                        <div style={{ fontSize: 9, color: sub }}>{a.progress_pct}%</div>
                      )}

                      {/* XP reward */}
                      {a.xp_reward > 0 && (
                        <div style={{ fontSize: 9, color: a.unlocked ? accent : sub }}>
                          +{a.xp_reward} XP
                        </div>
                      )}
                    </div>
