...
/**
 * Pure reconstruction of a session's detail from a ProgressMap.
 *
 * Shared by `useSessionDetail` (own sessions, map already in memory) and
 * `usePublicSessionDetail` (another user's session, map rebuilt from PB) so
 * both render identical data from a single implementation.
 */
...
export function useSessionDetail(
  ...
  return useMemo(
    () => buildSessionDetail(progress, date, workoutKey, exerciseCatalog),
