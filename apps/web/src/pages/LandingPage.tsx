...
function BenefitList({ items }: { items: string[] }) {
  return <ul className="mt-8 grid gap-3 border-t border-white/10 pt-6 text-sm leading-relaxed text-white/72">{items.map(item => <li key={item} className="flex gap-3"><Check className="mt-0.5 h-4 w-4 shrink-0 text-lime" /><span>{item}</span></li>)}</ul>
}
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[hsl(0_0%_3%)] text-white selection:bg-lime/30">
    ...
    <main>
      <section className="relative isolate flex min-h-[100svh] items-end overflow-hidden bg-[radial-gradient(ellipse_at_78%_22%,hsl(74_90%_57%_/_0.2),transparent_35%),radial-gradient(ellipse_at_78%_70%,hsl(0_0%_18%),transparent_38%),hsl(0_0%_3%)] px-6 pb-16 pt-32 md:px-10 md:pb-20">
        ...
        <div className="relative mx-auto grid w-full max-w-6xl items-end gap-10 lg:grid-cols-[1fr_.8fr]"><div className="max-w-2xl"><Eyebrow>{t('landing.kicker')}</Eyebrow><h1 className="mt-5 font-bebas text-[clamp(4.2rem,11vw,9rem)] leading-[.82] tracking-tight">{t('landing.heroTitle1')}<br /><span className="text-lime">{t('landing.heroTitle2')}</span></h1><p className="mt-8 max-w-lg text-lg leading-relaxed text-white/68 sm:text-xl">{t('landing.heroDesc')}</p><div className="mt-10 flex flex-col gap-3 sm:flex-row"><AndroidButton className="hero-android" /><WebButton onGetStarted={onGetStarted} className="hero-web" /></div></div><div className="relative mx-auto flex w-full max-w-md items-end justify-center lg:justify-end"><div aria-hidden="true" className="absolute inset-x-4 bottom-0 h-3/4 rounded-full bg-lime/15 blur-3xl" /><img src="/logo-bg-less.png" alt="" className="relative w-[min(82vw,430px)] drop-shadow-[0_28px_45px_rgba(0,0,0,.6)]" /><p className="absolute -bottom-1 left-0 max-w-[15rem] border-l border-lime pl-4 text-xs leading-relaxed text-white/65 sm:text-sm">{t('landing.heroProof')}</p></div></div>
      </section>
