...
| Property | Notes |
|---|---|
| `current_exercise_index` | 0-based index of the exercise being trained or rested on, counting only exercises with at least one set — the same count `set_logged`'s `exercise_index` uses. **Not** the raw session `stepIdx`: the session tracks progress per *set* (`buildSteps`, `session-machine.ts`), so an exercise with 2 sets occupies 2 consecutive `stepIdx` values; `current_exercise_index` maps that back to the exercise position (`findCurrentExerciseIndex`) so it stays comparable across exercises with different set counts. `0` is a real value (the first exercise), not "no data" — checked with `!= null`, never with truthiness. Present only when `stepIdx` resolves to a real exercise (see next row) |
