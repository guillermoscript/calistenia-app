...
const lsSetSettings = (d) => localStorage.setItem(LS_SETTINGS, JSON.stringify(d))
 ...
 *
 * @param {string|null} userId          — ID del usuario autenticado (de useAuth)
 * @param {string|null} activeProgramId — ID del programa activo (de usePrograms)
 */
export const useProgress = (userId = null, activeProgramId = null) => {
  const [progress, setProgress] = useState({})
  const [settings, setSettingsState] = useState({ phase: 1, startDate: null, weeklyGoal: 5 })
  ...
  useEffect(() => {
    ...
    if (lastUserId.current !== userId) {
      ...
      setProgress({})
      setSettingsState({ phase: 1, startDate: null, weeklyGoal: 5 })
      setPbReady(false)
    }
  ...
  const loadFromPB = async (uid) => {
    try {
      ...
      try {
        const settingsRec = await pb.collection('settings').getFirstListItem(
          pb.filter('user = {:uid}', { uid })
        )
        const s = {
  ...
  // ─── logSet ──────────────────────────────────────────────────────────────
  const logSet = useCallback(async (exerciseId, workoutKey, setData) => {
    const date = new Date().toISOString().split('T')[0]
    const key = `${date}_${workoutKey}_${exerciseId}`

    // Siempre guardar en localStorage (cache inmediato)
    setProgress(prev => {
      const existing = prev[key] || { sets: [], date, workoutKey, exerciseId }
      const updated = { ...existing, sets: [...existing.sets, { ...setData, timestamp: Date.now() }] }
  ...
  // ─── markWorkoutDone ─────────────────────────────────────────────────────
  const markWorkoutDone = useCallback(async (workoutKey, note = '') => {
    const date = new Date().toISOString().split('T')[0]
    const key = `done_${date}_${workoutKey}`

    setProgress(prev => {
      const newProg = { ...prev, [key]: { done: true, date, workoutKey, completedAt: Date.now(), note } }
      lsSet(newProg)
      return newProg
    })
  ...
  // ─── Helpers de consulta ─────────────────────────────────────────────────
  const isWorkoutDone = useCallback((workoutKey, date) => {
    const d = date || new Date().toISOString().split('T')[0]
    return !!progress[`done_${d}_${workoutKey}`]
  }, [progress])

  const getExerciseLogs = useCallback((exerciseId, limit = 10) =>
    Object.values(progress)
      .filter(v => v.exerciseId === exerciseId && v.sets)
      .sort((a, b) => b.date?.localeCompare(a.date))
      .slice(0, limit),
  [progress])

  const getWeeklyDoneCount = useCallback(() => {
  ...
  const getLastSessionDate = useCallback(() => {
    ...
    if (usePB && userId) {
      try {
        ...
        pb.collection('settings').create({
          ...
          pr_pistol:    updated.pr_pistol     ?? null,
          pr_handstand: updated.pr_handstand  ?? null,
        }).catch(e => console.warn('PB settings create error:', e))
      }
    }
