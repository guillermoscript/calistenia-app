 ...
 */
function buildPhases(phaseRecords) {
  return [...phaseRecords]
 ...
 */
function buildWeekDays(exerciseRecords) {
 ...
 */
function buildWorkoutsMap(exerciseRecords) {
  const map = {}
  exerciseRecords.forEach(r => {
...
export function usePrograms(userId = null) {
  const [programs, setPrograms]           = useState([])
  const [activeProgram, setActiveProgram] = useState(null)
  const [phases, setPhases]               = useState(FALLBACK_PHASES)
  const [weekDays, setWeekDays]           = useState(FALLBACK_WEEK_DAYS)
  ...
  const loadFromPB = async (uid) => {
    ...
    const catalog = catalogRes.items.map(p => ({
      id:             p.id,
      name:           p.name,
  ...
  // ── getWorkout ───────────────────────────────────────────────────────────
  const getWorkout = useCallback((phaseNumber, dayId) => {
    ...
    return fallbackGetWorkout(phaseNumber, dayId)
  }, [workoutsMap])

  // ── selectProgram ────────────────────────────────────────────────────────
  const selectProgram = useCallback(async (programId) => {
