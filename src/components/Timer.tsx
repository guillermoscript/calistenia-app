...
export default function Timer({ initialSeconds = 60, onComplete, autoStart = false, label = "Descanso" }: TimerProps) {
  ...
  useEffect(() => {
    if (running && seconds > 0) {
      // Warning at 10s
      if (seconds === 10) { playWarning(); vibrate([100]) }
      // Countdown ticks at 3, 2, 1
      if (seconds <= 3) { playCountdownTick(); vibrate([50]) }
  ...
  const reset = () => { if (intervalRef.current) clearInterval(intervalRef.current); setSeconds(initialSeconds); setRunning(false); setDone(false) }
