...
function lsLoadChecks() {
  try { return JSON.parse(localStorage.getItem(LS_KEY) || '[]') } catch { return [] }
}
function lsSaveCheck(check) {
  const checks = lsLoadChecks()
  checks.push(check)
  localStorage.setItem(LS_KEY, JSON.stringify(checks))
}

const PAIN_LABELS = { 1: 'Sin dolor', 2: 'Leve', 3: 'Moderado', 4: 'Fuerte', 5: 'Intenso' }
...
export default function LumbarCheckModal({ user, onDone, onSkip }) {
  const [step, setStep]             = useState(0)
  const [lumbarScore, setLumbarScore] = useState(null)
  const [sleptWell, setSleptWell]   = useState(null)
