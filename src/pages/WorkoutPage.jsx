        ...
        >
          <div className="[&:has(>*:last-child:not([data-overflow]))]:mask-none flex gap-2 overflow-x-auto pb-1 flex-nowrap scrollbar-none md:flex-wrap md:overflow-visible md:pb-0 md:[mask-image:none] md:[webkit-mask-image:none]">
            {PHASES.map(p => {
              ...
              const pa = { 1: 'border-lime text-lime', 2: 'border-sky-500 text-sky-500', 3: 'border-pink-500 text-pink-500', 4: 'border-amber-400 text-amber-400' }[p.id] || ''
              return (
                <Button
                  key={p.id}
      ...
      {workout ? (
        <div>
          ...
          <div className={cn(
            ...
            DAY_TYPE[selectedDayType]?.border || 'border-l-border'
          )}>
          ...
          <div className="flex flex-col gap-3">
            {workout.exercises.map(ex => (
              <ExerciseCard key={ex.id} exercise={ex} workoutKey={workoutKey}
                onLogSet={onLogSet} onStartRest={s => setRestTime(s)} logs={getExerciseLogs(ex.id)} />
            ))}
          </div>
        </div>
      ) : (
