...
function WorkDayClock() {
  ...
  const pauseCount25 = pauses.filter(p => p.type === '25').length
  const pauseCount60 = pauses.filter(p => p.type === '60').length
...
function CountdownPill({ label, seconds, accentClass, urgentBg, urgentBorder }) {
...
// ─── Root LumbarPage ──────────────────────────────────────────────────────────
export default function LumbarPage({ user }) {
